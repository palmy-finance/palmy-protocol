...
import {
  deployGenericLTokenImpl,
  deployGenericLTokenRev2Impl,
  deployGenericLTokenRev3Impl,
...
makeSuite('upgrade-ltoken', (testEnv: TestEnv) => {
  describe('LendingPoolConfigurator#updateLToken', () => {
    ...
    it('Prerequisite', async () => {
      ...
      expect(name).to.eq('Oasyslend interest bearing DAI');
    ...
    const generateUpdateLTokenInputParams = async (
      newLTokenImplAddress: string
    ): Promise<UpdateLTokenInputParams> => ({
      asset: _dai.address,
      treasury: createRandomAddress(),
      incentivesController: createRandomAddress(),
      name: 'Oasyslend interest bearing DAI UPDATED',
      symbol: 'lDAI UPDATED',
    ...
    it('fail if using lower version implementation', async () => {
      ...
      expect(name).not.to.eq('Oasyslend interest bearing DAI UPDATED');
    ...
    it('success', async () => {
      ...
      await expect(_configurator.updateLToken(updateLTokenInputParams))
        .to.emit(_configurator, 'LTokenUpgraded')
        .withArgs(_dai.address, reserveData.lTokenAddress, newLTokenImpl.address);
      ...
      expect(_name).to.eq('Oasyslend interest bearing DAI V3');
