...
task('check:oracle:fallback-oracle', 'check:oracle:fallback-oracle').setAction(
  async ({}, localBRE) => {
...
task(
  'check:oracle:price-aggregator-adapter-dia-impl',
  'check:oracle:price-aggregator-adapter-dia-impl'
  ...
  for (let i = 0; i < keys.length; i++)
    console.log(
      `${keys[i]} ... ${ethers.utils.formatUnits(await _oracle.currentPrice(values[i]), 8)}`
    );
});

const ABI_DIA_ORACLE_V2 = [
  ...
  },
  {
    ...
    name: 'getValue',
    inputs: [{ type: 'string', name: 'key', internalType: 'string' }],
  },
  {
    type: 'function',
    ...
    inputs: [
      ...
      { type: 'uint128', name: 'value', internalType: 'uint128' },
      { type: 'uint128', name: 'timestamp', internalType: 'uint128' },
    ],
  },
  {
    type: 'function',
