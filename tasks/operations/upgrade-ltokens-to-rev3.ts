...
import { deployGenericLTokenRev3ImplWithSigner } from '../../helpers/contracts-deployments';
import {
...
const SUPPORTED_NETWORK = ['oasys', 'shiden', 'localhost'] as const;
...
task(
  ...
  const pConf = loadPoolConfig(ConfigNames.Oasyslend);
  const protocolDataProvider = await getOasyslendProtocolDataProvider(
    constants.OasyslendProtocolDataProvider
  );
  ...
  if (assetAddress.toLowerCase() != target.asset.toLowerCase())
    throw new Error('Not equal input asset address to getted asset address');
  const { lTokenAddress } = await protocolDataProvider.getReserveTokensAddresses(assetAddress);
  if (lTokenAddress.toLowerCase() != target.ltoken.toLowerCase())
    throw new Error('Not equal input ltoken address to getted ltoken address');
