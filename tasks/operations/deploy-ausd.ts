...
const getReserveAssetAddresss = (network: eNetwork) => ({
  AUSD: marketConfigs.OasyslendConfig.ReserveAssets[network][SYMBOLS.AUSD],
});
...
task('external:deploy-ausd:setup-oracles').setAction(async ({ verify }, localBRE) => {
  ...
  await priceOracle.submitPrices([reserveAssetAddresss.AUSD], [INITIAL_PRICES.AUSD]);
  ...
  await priceOracleDiaImpl.setAssetSources([reserveAssetAddresss.AUSD], [SYMBOLS.AUSD]);
});
