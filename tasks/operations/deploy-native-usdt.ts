...
const getReserveAssetAddresss = (network: eNetwork) => ({
  NativeUSDT: marketConfigs.OasyslendConfig.ReserveAssets[network]['NativeUSDT'],
});
...
task('external:deploy-usdt:setup-oracles').setAction(async ({ verify }, localBRE) => {
  ...
  await priceOracle.submitPrices([reserveAssetAddresss.NativeUSDT], [INITIAL_PRICES.NativeUSDT]);
  ...
  await priceOracleDiaImpl.setAssetSources([reserveAssetAddresss.NativeUSDT], [SYMBOLS.NativeUSDT]);
});
